...
from dataset.dataset import Dataset_ssl_lits2017_png_train
...
def parse_args():
    ...
    parser.add_argument('--model_name', default='Unet',
                        choices=['Unet', 'AttUnet', 'res_unet_plus', 'R2Unet', 'R2AttU_Net',
                                 'sepnet', 'KiU_Net', 'Unet3D', 'ResT', 'ASSNet','LiNet'
                                 ])
    # pre trained
    ...
    parser.add_argument('--loss', default='BCEDiceBoundaryLoss',choices=['BCEDiceLoss', 'BCEDiceBoundaryLoss'])
...
def train(args, train_loader, model, criterion, optimizer, lr_decay, epoch, index):
    ...
    model.train()
    l2_reg = 0.5
    for i, (input, target, liver_dist_maps, tumor_dist_maps) in tqdm(enumerate(train_loader), total=len(train_loader)):
        # compute gradient and do optimizing step
        optimizer.zero_grad()
        input = input.cuda(non_blocking=True)
        target = target.cuda(non_blocking=True)
        liver_dist_maps = liver_dist_maps.cuda(non_blocking=True)
        tumor_dist_maps = tumor_dist_maps.cuda(non_blocking=True)
        ...
        outputs = model(input)
        loss = criterion(outputs, target, liver_dist_maps, tumor_dist_maps)
...
def main():
    ...
    if not os.path.exists('/data/3DUNET_Github/20250221/AFFSegNet/trained_model/{}_{}/{}'.format(args.dataset, args.model_name, timestamp)):
        os.makedirs('/data/3DUNET_Github/20250221/AFFSegNet/trained_model/{}_{}/{}'.format(args.dataset, args.model_name, timestamp))
    print('Config -----')
    ...
    with open('/data/3DUNET_Github/20250221/AFFSegNet/trained_model/{}_{}/{}/args.txt'.format(args.dataset,args.model_name, timestamp), 'w') as f:
        for arg in vars(args):
            print('%s: %s' % (arg, getattr(args, arg)), file=f)

    joblib.dump(args, '/data/3DUNET_Github/20250221/AFFSegNet/trained_model/{}_{}/{}/args.pkl'.format(args.dataset, args.model_name, timestamp))
    ...
    if args.model_name == 'Unet3D':
        ...
        # 加载训练集图片和掩码路径
        train_img_paths = glob('/data/3DUNET_Github/20250228/AFFSegNet/data/trainImage/*')
        train_mask_paths = glob('/data/3DUNET_Github/20250228/AFFSegNet/data/trainMask/*')
        train_liver_dist_paths = glob('/data/3DUNET_Github/20250228/AFFSegNet/data_boundary/train_liver/*')
        train_tumor_dist_paths = glob('/data/3DUNET_Github/20250228/AFFSegNet/data_boundary/train_tumor/*')

        val_img_paths = glob('/data/3DUNET_Github/20250228/AFFSegNet/data/valImage/*')
        val_mask_paths = glob('/data/3DUNET_Github/20250228/AFFSegNet/data/valMask/*')
        #val_liver_dist_paths = glob('/data/3DUNET_Github/20250221/AFFSegNet/data_boundary/val_liver/*')
        #val_tumor_dist_paths = glob('/data/3DUNET_Github/20250221/AFFSegNet/data_boundary/val_tumor/*')

        # 打印数据集大小
        print("Train dataset size: ", len(train_img_paths))
        print("Validation dataset size: ", len(val_img_paths))
    ...
    if args.model_name == 'Unet':
        ...
        if args.pretrained == True:
            pretrained_path = '/data/3DUNET_Github/20250227/AFFSegNet/trained_model/LiTS2017_Unet/2025-02-22-01-05-12/epoch49-val_loss:0.2434-val_dice2:0.7456_model.pth'
            print('U_Net pretrained was pretrained selected!')
            #Unet_path = '/data/3DUNET_Github/20250222/AFFSegNet/trained_model/LiTS2017_Unet/2025-02-22-01-05-12/epoch49-val_loss:0.2434-val_dice2:0.7456_model.pth'
    ...
    train_dataset = Dataset_ssl_lits2017_png_train(args, train_img_paths, train_mask_paths, train_liver_dist_paths,
                                             train_tumor_dist_paths, transform=transform_ct)
    ...
    train_criterion = losses.BCEDiceBoundaryLoss(boundary_weight=0.001).cuda()  # 训练集使用
    val_criterion = losses.BCEDiceLoss_lits2017().cuda()

    for i, epoch in enumerate(range(args.epochs)):
        print('Epoch [%d/%d]' % (epoch, args.epochs))
        train_log = train(args, train_loader, model, train_criterion, optimizer, scheduler_mult, epoch, i)
        ...
        if val_trigger == True:
            ...
            val_trigger = False
            val_log = validate(args, val_loader, model, val_criterion)
            ...
            log.to_csv('/data/3DUNET_Github/20250221/AFFSegNet/trained_model/{}_{}/{}/Validation_{}_{}_{}_batchsize_{}.csv'.format(args.dataset, args.model_name, timestamp, args.model_name,
                                                                                 args.aug, args.loss, args.batch_size),index=False)
            print('save result to csv ->')
            torch.save(model.state_dict(),
                       '/data/3DUNET_Github/20250221/AFFSegNet/trained_model/{}_{}/{}/epoch{}-val_loss:{:.4f}-val_dice2:{:.4f}_model.pth'.format(
            ...
            # 使用 pd.concat 替代 append
            log = pd.concat([log, tmp.to_frame().T], ignore_index=True)
            log.to_csv('/data/3DUNET_Github/20250221/AFFSegNet/trained_model/{}_{}/{}/Train_{}_{}_{}_batchsize_{}.csv'.format(args.dataset, args.model_name, timestamp,
