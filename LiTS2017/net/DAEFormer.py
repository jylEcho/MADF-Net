...
class DAEFormer(nn.Module):
    def __init__(self, num_classes=9, head_count=1, token_mlp_mode="mix_skip"):
        ...
        # Encoder
        dims, key_dim, value_dim, layers = [[128, 320, 512], [128, 320, 512], [128, 320, 512], [4, 4, 4]]
        self.backbone = MiT(
            image_size=224,
            in_dim=dims,
            key_dim=key_dim,
...
if __name__ == '__main__':
    ...
    #
    # # 计算层数，这里我们特别关注Transformer编码器层
    # num_transformer_layers = len(model.blocks)
    # # 创建模型实例
    # unet = DAEFormer()
    #
    # # 假设输入图像的尺寸为 224x224
    # input_tensor = torch.rand(1, 1, 224, 224)  # 1个通道的图像
    # num_layers = sum(1 for _ in unet.modules())
    # # 获取输出
    ...
    #
    # # 创建MiT实例
    # mit_instance = MiT(image_size=224, in_dim=[128, 320, 512], key_dim=[128, 320, 512],
    #                    value_dim=[128, 320, 512], layers=layers, head_count=1, token_mlp="mix_skip")
    #
    # # 计算编码器层数
    # encoder_layers = sum(len(block) for block in [mit_instance.block1, mit_instance.block2, mit_instance.block3])
    # print(encoder_layers)
    from thop import profile

    model =  DAEFormer().cuda('cuda:1')
    # model.apply(weights_init)
    x = torch.rand(1, 3, 224, 224).cuda('cuda:1')
    # z = torch.randn(1,64,64,65)
    # se = SEBlock(64)
    # print(se(z))
    # 使用 thop 计算 FLOPs
    flops, params = profile(model, inputs=(x,))

    print(f"FLOPs: {flops / 1e9} G FLOPs")  # 以十亿（Giga）FLOPs为单位打印
