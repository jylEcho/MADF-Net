...
class Encoder(nn.Module):
    def __init__(self):
        super().__init__()
        self.base = models.resnext50_32x4d(pretrained=False)
...
if __name__ == '__main__':
    ...
    model.cuda()
    model.train()
    x_pv = torch.randn(2, 3, 224, 224).cuda()
    x_art = torch.randn(2, 3, 224, 224).cuda()
    final_seg, initial_seg = model(x_pv, x_art)
    print(final_seg.shape)
    # 计算参数量和flops
    from thop import profile
    flops, params = profile(model, inputs=(x_pv, x_art))
