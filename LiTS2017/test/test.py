...
if model change
'''
model_path = '/data/3DUNET_Github/20250221/AFFSegNet/trained_model/LiTS2017_Unet/2025-03-11-15-17-38/epoch0-val_loss:0.7291-val_dice2:0.5404_model.pth'
csv_path = '/data/3DUNET_Github/20250221/AFFSegNet/test/pred5404'
save_pred_png_trigger = True
pred_png_path = '/data/3DUNET_Github/20250221/AFFSegNet/test/pred5404'
...
def main():
    ...
    # Data loading
    img_paths = sorted(glob('/data/3DUNET_Github/20250221/AFFSegNet/data/testImage_lits2017_png/*'))
    mask_paths = sorted(glob('/data/3DUNET_Github/20250221/AFFSegNet/data/testMask_lits2017_png/*'))
        ...
        for i, (input, target) in tqdm(enumerate(test_loader), total=len(test_loader)):
            ...
            if save_pred_png_trigger == True:
                for idx in range(output.shape[0]):
                    ...
                    original_filename = os.path.basename(img_paths[i * args.batch_size + idx])
                    filename, _ = os.path.splitext(original_filename)  # 去掉扩展名
                    filename += '_pred.png'  # 添加预测结果的后缀
