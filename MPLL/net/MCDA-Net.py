...
if __name__ == '__main__':
    net = Network()
    x_dp = torch.randn(2, 1, 224, 224)
    x_vp = torch.randn(2, 1, 224, 224)
    x_ap = torch.randn(2, 1, 224, 224)
    x_pre = torch.randn(2, 1, 224, 224)
    out = net(x_dp, x_vp, x_ap, x_pre)
    print(out[0].shape)
    # 计算参数量和flops
    from thop import profile
    flops, params = profile(net, inputs=(x_dp, x_vp, x_ap, x_pre))
